...
{ 
    ...
    /**
     * constant for onInstall and onInstalling listeners
     * @type String 
     * @static
     */
    TYPE_INSTALL: 'S',
    ...
    {
        BrowserUtils.debug("Observing " + a_topic);
        ...
        // last browser window is closed
        else if (a_topic == "browser-lastwindow-close-requested"  ) 
        {               
            for (var pkg in this.d_uninstall)
            {         
                // if we have any pending uninstall callbacks, execute them now
            ...
            for (var pkg in this.d_obs[this.TYPE_LASTWINDOW])
            {
                // execute any registered application quit observer callbacks
                if (typeof this.d_obs[this.TYPE_QUIT][pkg] == 'function')
                {
                    this.d_obs[this.TYPE_LASTWINDOW][pkg]();
                }
            }
        }
    ...
    start: function()
    {
        ...
        {
            BrowserUtils.debug("AddonManager unavailable " + a_e);
        }
    },
    ...
    unregisterObserver : function(a_type) 
    {
        ...
        observerService.removeObserver(this,"browser-lastwindow-close-requested");
    ...
    /**
     * FF4 AddonManager Listener methods
     */
    onInstalling: function(a_addon, a_needsrestart)
    {
        BrowserUtils.debug("Listening to onInstalling " + a_addon.id);             
        // execute any registered install callbacks
        // do this only for Alpheios extensions
        var is_alpheios = false;
        var alph_pkgs = BrowserUtils.getAlpheiosPackages();
        alph_pkgs.forEach(
            function(a_pkg)
            {
                if (a_addon.id == a_pkg.id)
                {
                    is_alpheios = true;
                }
            }
        ...
        if (is_alpheios)
        {   
            for (var pkg in this.d_obs[this.TYPE_INSTALL])
            {
                this.d_obs[this.TYPE_INSTALL][pkg]();            
            }
    ...
    onInstalled: function(a_addon)
    {
        BrowserUtils.debug("Listening to onInstall of " + a_addon.id);     
        // execute any registered install callbacks
        // do this only for Alpheios extensions
        var is_alpheios = false;
        var alph_pkgs = BrowserUtils.getAlpheiosPackages();
    ...
    onUnInstalled: function(a_addon)
    {
        BrowserUtils.debug("Listening to install of " + a_addon.id); 
        // if a package with a registered uninstall observer is being uninstalled
        // copy the callback reference to the d_uninstall array pending the 
        // application restart
        this.d_uninstall[a_addon.id] = this.d_obs[this.TYPE_UNINSTALL][a_addon.id];        
    },
    
    onOperationCancelled: function(a_addon)
    {
            if ((a_addon.pendingOperations & AddOnManager.PENDING_UNINSTALL) &&
                this.d_uninstall[a_addon.id])
            {
                this.d_uninstall[a_addon.id] = false;
            }
    }

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template name="inflection-set">
        ...
        <!-- if non-empty set -->
        <!-- ignore conjunctions, prepositions, and interjections -->
        <!-- since inflection adds nothing to dict info for those -->
        <xsl:if
    ...
    <xsl:template name="item-plus-text-plus-context">
        ...
        <xsl:if test="$item">
            <span class="mp-{name($item)}" context="{translate($item,' ','_')}">

...
<br/>
  ...
  <xsl:template name="sense_vote">
	  ...
	  <xsl:element name="a">
		  ...
		  <xsl:value-of select="concat('submitvote.jsp?type=sense&amp;lexquery=',$lexquery,'&amp;doc=',$source_id,'&amp;subquery=',$subquery,'&amp;form=',$form,'&amp;which=',$which,'&amp;sense_id=',@id,'&amp;lang=',$voting_lang)" />
	      </xsl:attribute>
	      [select]
	  </xsl:element>
      </xsl:if>
  ...
  <xsl:template match="figure[@n!='' and not(descendant::p)]">
    ...
    <p><table width="120" align="{$align}">
      <tr><td>
        <a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}"><img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}&amp;thumb=1"/></a>
      </td></tr>
  ...
  <xsl:template match="figure[@n!='' and (descendant::p)]">
    <xsl:choose>
      <xsl:when test="ancestor::table">
        <table align="center">
          <tr><td><a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}"><img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}&amp;thumb=1"/></a></td>
  ...
  <xsl:template match="figure[not(descendant::p)]">
    ...
    <xsl:variable name="imID"><xsl:value-of select="$document_id"/><xsl:text>.fig</xsl:text><xsl:value-of select="$figID"/></xsl:variable>
    <xsl:variable name="align">
      <xsl:call-template name="figalign"/>
    </xsl:variable>
    <p><table width="120" align="{$align}">
      <tr><td>
        <a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}"><img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}&amp;thumb=1"/></a>
      </td></tr>
      <tr><td><xsl:apply-templates /></td></tr>
    </table></p>
  </xsl:template>
  ...
  <xsl:template match="figure[descendant::p]">
    ...
    <xsl:choose>
      <xsl:when test="ancestor::table">
        <table align="center">
          <tr><td><a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}"><img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}&amp;thumb=1"/></a></td>
          <td><xsl:apply-templates /></td></tr>
        </table>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="note[@place='marg']">  
    ...
    <table width="100" align="{$align}"><tr><td bgcolor="#f0f0f0">
      <span class="sidetext">
      <xsl:apply-templates />
      </span>
    </td></tr></table>
  ...
  <xsl:template match="pb">
        ...
        <xsl:variable name="imID"><xsl:value-of        select="$document_id"/><xsl:text>.</xsl:text><xsl:value-of
        select="$figID"/></xsl:variable>
	<xsl:choose>
	  ...
	  <xsl:when test="@id and starts-with($document_id, 'Perseus:text:2001.05.0007')">
		[<a target="figure"
        href="http://repository01.lib.tufts.edu:8080/fedora/get/tufts:perseus.image.2001.05.0007.{$figID}/bdef:TuftsImage/getMediumRes/"><xsl:value-of
        select="./@n"/></a>]   		
	  </xsl:when>	
	  <xsl:when test="@id and starts-with($document_id, 'Perseus:text:2001.05.0044')">
		[<a target="figure"
        href="http://repository01.lib.tufts.edu:8080/fedora/get/tufts:perseus.image.2001.05.0044.{$figID}/bdef:TuftsImage/getMediumRes/"><xsl:value-of
        select="./@n"/></a>]   		
	  </xsl:when>	
	  <xsl:when test="@id and starts-with($document_id, 'Perseus:text:2001.05')">
		[<a target="figure"
        href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}"><xsl:value-of
        select="./@n"/></a>]   		
	  </xsl:when>
	  <xsl:when test="@id">
  ...
  <xsl:template name="bibl-link">
	...
	<xsl:when test="@n=concat($sourcework,':',$sourcesub)">
	    <span style="font-size: x-large;"><a href="text.jsp?doc={@n}&amp;lang=original" target="_new"><xsl:call-template name="language-filter">
              <xsl:with-param name="lang" select="@lang" />
	      <xsl:with-param name="default" select="'en'" />
            </xsl:call-template></a></span>
	</xsl:when>
      <xsl:when test="@n and $sourcework != 'none' and starts-with(@n, $sourcework)">
	  <b><a href="text.jsp?doc={@n}&amp;lang=original" target="_new"><xsl:call-template name="language-filter">
            <xsl:with-param name="lang" select="@lang" />
	    <xsl:with-param name="default" select="'en'" />
  ...
  <xsl:template match="ref[@target!='']">
    <a href="text.jsp?doc={$document_id}:id={@target}"><xsl:call-template name="language-filter">
      <xsl:with-param name="lang" select="@lang" />
    </xsl:call-template></a>
  </xsl:template>

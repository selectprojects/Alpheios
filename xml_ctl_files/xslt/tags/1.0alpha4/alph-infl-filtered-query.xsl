...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
            ...
            <xsl:if test="generate-id(.) = generate-id($firstgroup[.=current()])">
              ...
              <tr id="header-{$lastgroup1}" class="group1row">
                <xsl:element name="th">
                  ...
                  <!--xsl:call-template name="add-footnote">
                    <xsl:with-param name="a_item" select="/infl-data/order-table/order-item[@attname=$groupheader]"/>
                  </xsl:call-template-->
                </xsl:element>
                  ...
                  <xsl:if test="count($thirdgroup) = 0">
                    ...
                    <tr class="data-row" id="{$rowId}"> <!-- start new row -->
                      ...
                      <xsl:call-template name="rowgroup">
                        ...
                        <xsl:with-param name="a_context" select="$e_group2"/>
                  ...
                  <xsl:for-each select="$thirdgroup">
                    ...
                    <xsl:if test="generate-id(.) = generate-id($thirdgroup[.=current()])">
                      ...
                      <tr class="data-row" id="{$rowId}"> <!-- start new row -->
                        <xsl:choose>
                          <xsl:when test="position()=1">
                            ...
                            <xsl:element name="th">
                              ...
                              <xsl:attribute name="context"><xsl:value-of select="$e_group2"/></xsl:attribute>
                              <xsl:attribute name="{concat('alph-',$e_group2)}">
                                <xsl:value-of select="
                        ...
                        <xsl:call-template name="rowgroup">
                          <xsl:with-param name="a_data" select="$data"/>
                          <xsl:with-param name="a_context" select="$e_group3"/>
    ...
    <xsl:for-each select="$group4Vals">
      ...
      <xsl:if test="position()=1">
        ...
        <xsl:element name="th">
          ...
          <xsl:attribute name="context"><xsl:value-of select="$a_context"/></xsl:attribute>
          <xsl:attribute name="{concat('alph-',$a_context)}">
            <xsl:value-of select="
  ...
  <xsl:template name="headers">
    ...
    <tr id="filterrow">
      <th colspan="2">
        <span class="header-text"><xsl:value-of select="$e_filterKey"/></span> :
        <span class="header-text"><xsl:value-of select="$e_filterValue"/></span>
      </th>
      <xsl:call-template name="headerrow">
        <xsl:with-param name="a_headerrow1" select="$a_headerrow1"/>
        <xsl:with-param name="a_headerrow2" select="$a_headerrow2"/>
        <xsl:with-param name="a_headerrow3" select="$a_headerrow3"/>
      </xsl:call-template>
    </tr>
    <tr id="headerrow1">

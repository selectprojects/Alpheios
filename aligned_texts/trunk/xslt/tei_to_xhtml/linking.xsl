      ...
      <xsl:when test="starts-with(local-name(.),'div')">
        ...
        <xsl:variable name="xpath">
          <xsl:for-each select="ancestor-or-self::*">
              <xsl:choose>
                <!-- alpheios addition -->
                  <xsl:when test="@n and @type and
                    $alpheiosNTypeIDs='true'">
                    <xsl:value-of select="concat(translate(@type,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),@n)"/>
                  </xsl:when>
                  <xsl:otherwise>
        ...
        <xsl:value-of select="substring-after($xpath,'TEI.2.1_text.1_body.1_')"/>
      </xsl:when>

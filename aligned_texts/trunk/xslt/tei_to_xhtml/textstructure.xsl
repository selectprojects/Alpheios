...
-->
  ...
  <xsl:template match="*" mode="generateNextLink">
    <!-- alpheios addition -->
    <xsl:choose>
      <xsl:when test="$alpheiosPedagogicalText = 'true'">
        ...
        <a class="navigation">
          ...
          <xsl:element name="img">
            ...
            <xsl:attribute name="src"><xsl:value-of select="$alpheiosSiteBaseUrl"/><xsl:text>/img/next.gif</xsl:text></xsl:attribute>
          </xsl:element>
        </a>    
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$alpheiosPedagogicalText = 'true'">
        ...
        <a class="navigation">
          ...
          <xsl:element name="img">
            <xsl:attribute name="alt"><xsl:value-of select="$alt_text"/></xsl:attribute>
            <xsl:attribute name="src"><xsl:value-of select="$alpheiosSiteBaseUrl"/><xsl:text>/img/prev.gif</xsl:text></xsl:attribute>
          </xsl:element>
        </a>    
      </xsl:when>
      <xsl:otherwise>
		...
		<xsl:with-param name="mode" select="local-name(.)"/>
          ...
          <xsl:otherwise>
            <html>
              ...
              <body id="TOP" onload="check_for_alpheios();">
  ...
  <xsl:template match="TEI.2">
    ...
    <html>
      ...
      <body class="simple" id="TOP" onload="check_for_alpheios();">
        <xsl:call-template name="bodyHook"/>
        <xsl:call-template name="bodyJavascriptHook"/>
  ...
  <xsl:template
    ...
    <xsl:message>Depth=<xsl:value-of select="$depth"/></xsl:message>
    <xsl:message>Split level=<xsl:value-of select="$splitLevel"/></xsl:message>
  ...
  <xsl:template name="doDivBody">
    ...
    <xsl:if test="$alpheiosHasAlignment = 'true'">
      <xsl:variable name="trans_url">
        <xsl:value-of select="$alpheiosSiteBaseUrl"/>
        <xsl:text>/</xsl:text>
        <xsl:value-of select="translate(/TEI.2/@id,'_','.')"/>
        <xsl:text>/align/</xsl:text>
      </xsl:variable>
      <xsl:variable name="src_lang">
        <xsl:call-template name="addLangAtt"/>
      </xsl:variable>
      <div id="alph-trans-url" 
               url="{concat($trans_url,$ident,$outputSuffix)}"/>
    </xsl:if>
    <xsl:choose>
      <xsl:when test="parent::div/@rend='multicol'">
        <td valign="top">
  ...
  <xsl:template name="includeCSS">
    ...
    <xsl:if test="not($alpheiosCss='')">
      <link href="{$alpheiosCss}" rel="stylesheet" type="text/css"/>
    </xsl:if>
  ...
  <xsl:template name="includeJavascript">
    <xsl:if test="$skipJavascript='false'">
    <xsl:text>&#10;</xsl:text>
  ...
  <xsl:template name="stdfooter">
    ...
    <xsl:variable name="alpheiosignore">
      <xsl:if test="$alpheiosPedagogicalText='true'">
        <xsl:text>alpheios-ignore</xsl:text>
      </xsl:if>
    </xsl:variable>
  ...
  <xsl:template name="topNavigation">
    <xsl:if test="ancestor::teiCorpus">
      ...
      <xsl:choose>
        <xsl:when test="$alpheiosPedagogicalText='true'">
          <xsl:if test="not($alpheiosSkipNav='true')">

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
        ...
        <xsl:choose>
            <xsl:when test="$fragment">
                <xsl:call-template name="verbtable">
                    <xsl:with-param name="endings" select="//verb-ending-set"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <html>
                    <head>
                        <link rel="stylesheet" type="text/css" href="mp-verb-conj-group.css"/>
                    </head>
                    <body>
                        <xsl:call-template name="verbtable">
                            <xsl:with-param name="endings" select="//verb-ending-set"/>
    ...
    <xsl:template name="verbtable">
        ...
        <table id="mp-infl-table"> <!-- start verb table -->
            <caption>
    ...
    <xsl:template name="rowgroup">
        ...
        <xsl:for-each select="$data">
            ...
            <xsl:sort 
                ...
                data-type="number"/>
            <xsl:sort 
                select="/conjdata/order-table/order-item[@attname=$group6 
            ...
            <td>
                <xsl:variable name="verb-endings" select="verb-ending"/>
                <xsl:for-each select="$verb-endings">
                    <xsl:variable name="entries" select="count($selected_endings/div[@class='mp-entry'])"/>                    
                    <xsl:variable name="selected" 
                        select="$selected_endings
                            [
                              (div[@class='mp-dict']/span[@class='mp-conj']/@context = current()/../@conj)
                              and
                              (div[@class='mp-infl-set']/
                              div[
                              @class='mp-infl' 
                              and (span[@class='mp-tense']/text() = current()/../@tense)
                              and (span[@class='mp-voice']/text() = current()/../@voice)
                              and (span[@class='mp-mood']/@context = current()/../@mood)
                              and (span[@class='mp-pers']/@context = current()/../@person)
                              and (span[@class='mp-num']/@context = current()/../@num)
                              ])
                            ]"/>    

/**
 * @fileoverview Handler for the Melampus Declension Table window
 * @version $Id $
 * 
...
MP.decl = {
    ...
    onLoad: function() {
        ...
        document.getElementById("mp-decl-browser")
            .addEventListener(
                "DOMContentLoaded",
                function() 
                {
                    var tbl = $('#declension-table',this.contentDocument);
                    MP.decl.showCols(tbl,link_target,casetypes);
                    MP.decl.resize_window(this.contentDocument);
                },
                true
                );
                
        var decl_browser = $("#mp-decl-browser");
        ...
        var url =
            ...
            '/content/declensions/' + 
            "mp-decl-" + link_target.showcase + ".html";
    ...
    showCols: function(a_tbl,a_link_target, a_casetypes) {
        ...
        if ( a_link_target.suffixes[showcase] != null 
            ...
            //MP.util.log(a_link_target.word + ": col_parents: " + col_parents.join(","));
            ...
            {
                MP.decl.expand_table(a_tbl,topdoc);
            }
            else 
            {
            ...
            if (a_casetypes[i] != a_link_target.showcase)
            {
                var link =
                   topdoc.createElementNS("http://www.w3.org/1999/xhtml",
                    ...
                    link.setAttribute("class","mp-decl-table-link");
                    link.setAttribute("mp-case-link",a_casetypes[i]);
                    link.innerHTML = 
                        document
        ...
        $(".mp-decl-table-link",topdoc).click(
            function(e) {
                var showcase = $(this).attr("mp-case-link");
                window.opener.MP.main.getLanguageTool().showDeclensionTable(e,showcase);
            }
        );
    },
    ...
    onKeyDown: function(a_e) {
        if (a_e.keyCode == 16) {
            if (window.opener.MP.xlate.popupVisible()) {
                ...
                window.opener.MP.main.getLanguageTool().showDeclensionTable(a_e);
            }
        }
